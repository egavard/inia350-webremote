...
$(function () {
        ...
        try {
            ...
            socket.onopen = function () {
                ...
                socket.send(JSON.stringify({
                    ...
                    name: "offsets",
                    offsets: $.map(offsets, function (val, key) {
                        return {
                            name: key,
                            address: val["offsetaddress"],
                            type: val["offsettype"],
                            size: val["offsetsize"]
                        }
                    })
    ...
    $('#openMenuAction').click(function () {
        openMenu(false)
    });
    $('#closeMenuAction').click(function () {
        closeMenu()
    });
    ...
    let updateLvars = function (data) {
        $.each(data, function (key, val) {
            if (lvars.hasOwnProperty(key)) {
                ...
                // ... SPOILERS
                if (key == "INI_SPOILERS_HANDLE_POSITION") {
                    $("#spoilerSlider").slider("value", 1 - val);
                }
                if(key == "INI_ECAM_ACTIVE_PAGE"){
                    console.log(key+"="+val)
                }
        ...
        if ($(".lvar_status_indicator[data-lvarname=INI_SPOILERS_ARMED]").hasClass("ui-state-active")) {
            $(".lvar_status_indicator[data-lvarname=SPOILERS_SET]").removeClass("ui-state-active");
        }
    };
    ...
    $("#connectionPanel").dialog({
        ...
        title: "WebRemote for INIBuilds A350",
        modal: true,
        draggable: false,
        resizable: false,
        closeOnEscape: false,
